...
import AllGeneral from './AllGeneral';
...
function General({ navigation }) {
    ...
    async function getLatestNews() {
        const url = `${base_url}/top-headlines?country=us&category=general&apiKey=${api_key}`;
        const options = {
            method: 'GET',
        };
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setLatesNews(data.articles.slice(0, 10))
            } catch (error) {
    ...
    return (
        <>
            <FlatList
                ...
                ListHeaderComponent={
                <TouchableOpacity onPress={()=> navigation.navigate("AllGeneral")} style={styles.topstories}>
                <Text style={styles.topStoriesHeading}>General</Text>
                <Image source={require('../img/right-arrow.png')} style={styles.arrowIcon}/>
                </TouchableOpacity>
                }
            />
        </>
