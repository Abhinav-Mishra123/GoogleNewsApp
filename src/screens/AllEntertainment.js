...
import AllBusinessNews from './AllBusinessNews';

function AllEntertainment({ navigation }) {

    const [latestNews, setLatesNews] = useState([])

    async function getLatestNews() {
        const url = `${base_url}/top-headlines?country=in&category=entertainment&apiKey=${api_key}`;
        const options = {
            method: 'GET',
        };
    ...
    function renderNewsItem ({ item, index })  {
        ...
        const truncatedTitle = item.title.length > 70 ? `${item.title}...` : item.title;
                ...
                <TouchableOpacity style={styles.newsCard} onPress={() => handleBlogPress(item.url)}>
                    <View style={styles.twoColumnContainer}>
                        <View style={styles.leftColumn}>
                            <Text style={styles.newsTitle}>{truncatedTitle}</Text>
    ...
    return (
        <>
            <FlatList
                ...
                ListHeaderComponent={
                <Text style={styles.topStoriesHeading}>All Entertainment Stories</Text>
                }
            />
        </>
    )
